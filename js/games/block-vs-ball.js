/* ============================================================
   BLOCK VS BALL v3.0 — OPTIMIZED MOBILE PERFORMANCE
   ============================================================ */
...
class BlockVsBall {
    constructor(canvas, onScore) {
        ...
        this.MAX_PARTS = this.isMobile ? 30 : 70;
        ...
        this.stars = this._mkStars(this.isMobile ? 15 : 35);
    ...
    _rrect(x, y, w, h, r) {
        ...
        c.beginPath(); c.moveTo(dx+dr, dy);
        c.arcTo(dx+dw, dy, dx+dw, dy+dh, dr); c.arcTo(dx+dw, dy+dh, dx, dy+dh, dr);
        c.arcTo(dx, dy+dh, dx, dy, dr); c.arcTo(dx, dy, dx+dw, dy, dr); c.closePath();
    }
    ...
    _sfx(name, cd=80) { if(!window.audioManager) return; try{ audioManager.play(name); }catch(e){} }

    _genLevel() {
        this.blocks = [];
        const rows = Math.min(5+Math.floor(this.level/2), 10);
        const cols = this.COLS, pad = 4;
        ...
        for (let r=0; r<rows; r++) {
            for (let c=0; c<cols; c++) {
                ...
                this.blocks.push({ x:ox+c*(bw+pad), y:oy+r*(bh+pad), w:bw, h:bh, ci, hp, maxHp:hp, hitAnim:0, scale:0, dead:false });
            }
        }
    }
    ...
    _resetBall() {
        ...
        this.balls = [{ x:this.paddle.x, y:this.paddle.y-12, dx:0, dy:0, r:6, speed:spd, attached:true, trail:[] }];
    }
    ...
    _handleTap() {
        if (this.state===this.STATE.WAIT) {
            ...
            this.balls.forEach(b => {
                if (b.attached) { b.attached=false; b.dx=(Math.random()-0.5)*3; b.dy=-b.speed; }
            });
            this._sfx('click'); return;
        }
        if (this.state===this.STATE.DEAD && this.deathA>0.8) { this._restart(); return; }
        this.balls.forEach(b => {
            if (b.attached) { b.attached=false; b.dx=(Math.random()-0.5)*3; b.dy=-b.speed; this._sfx('shoot'); }
        });
    }

    _restart() {
        this.score=0; this.onScore(0);
        this.level=1; this.lives=3; this.combo=0; this.maxCombo=0;
    ...
    _toggleFS() {
        if (window.NeonFS) { window.NeonFS.toggle(); }
            ...
            const el = this.canvas.parentElement || document.documentElement;
    ...
    update(ts, dt) {
        ...
        // Stars — skip on mobile every other frame
        if (!this.isMobile) this.stars.forEach(s => s.ph += s.sp);
        ...
        this.blocks.forEach(b => { b.scale=Math.min(1,b.scale+0.06); b.hitAnim=Math.max(0,b.hitAnim-0.06); });

        this.parts = this.parts.filter(p => {
            p.x+=p.vx; p.y+=p.vy; p.vy+=p.g; p.vx*=0.96; p.life-=p.dec; p.r*=0.96;
        ...
        for (let i=this.balls.length-1; i>=0; i--) {
            ...
            if (b.dy>0 && b.y+b.r>=this.paddle.y-this.paddle.h/2 && b.y-b.r<=this.paddle.y+this.paddle.h/2 && b.x>=this.paddle.x-pw && b.x<=this.paddle.x+pw) {
                const hit=(b.x-this.paddle.x)/pw;
                const angle=hit*Math.PI/3;
                const spd=Math.sqrt(b.dx*b.dx+b.dy*b.dy);
                b.dx=Math.sin(angle)*spd; b.dy=-Math.abs(Math.cos(angle)*spd);
                b.y=this.paddle.y-this.paddle.h/2-b.r-1;
                this.combo=0; this._sfx('bounce'); this.shakeT=3;
            }

            for (let j=this.blocks.length-1; j>=0; j--) {
                const bl=this.blocks[j]; if(bl.dead) continue;
                if (b.x+b.r>bl.x && b.x-b.r<bl.x+bl.w && b.y+b.r>bl.y && b.y-b.r<bl.y+bl.h) {
                    const oleft=(b.x+b.r)-bl.x, oright=(bl.x+bl.w)-(b.x-b.r);
                    const otop=(b.y+b.r)-bl.y, obot=(bl.y+bl.h)-(b.y-b.r);
                    ...
                    bl.hp--; bl.hitAnim=1;
                    if (bl.hp<=0) {
                        bl.dead=true; this.combo++; this.maxCombo=Math.max(this.maxCombo,this.combo);
                        const bc=this.BCOLORS[bl.ci], pts=bc.pts*Math.max(1,this.combo);
                        this.score+=pts; this.onScore(this.score);
                        if(this.score>this.bestScore){this.bestScore=this.score;localStorage.setItem('bvb2_best',this.bestScore);}
                        const cx=bl.x+bl.w/2, cy=bl.y+bl.h/2;
                        this._burst(cx,cy,bc.fill,this.isMobile?6:10);
                        this.rings.push({x:cx,y:cy,r:bl.w*0.4,a:0.65,col:bc.fill});
                        this.pops.push({x:cx,y:cy-10,text:this.combo>1?`+${pts} ×${this.combo}`:`+${pts}`,col:this.combo>3?'#FFD700':bc.fill,life:1000,op:1});
                        this.shakeT=Math.min(10,this.combo*2);
                        this.flashA=Math.min(0.18,0.04+this.combo*0.02); this.flashC=bc.fill;
            ...
            if (b.y>this.H+20) this.balls.splice(i,1);
        }

        if (this.balls.length===0) {
            this.lives--; this.combo=0; this.shakeT=14;
            this.flashA=0.4; this.flashC='#FF0044'; this._sfx('fail');
            if(this.lives<=0) this._gameOver();
            else { this._resetBall(); this.state=this.STATE.WAIT; }
        }

        if (this.blocks.every(b=>b.dead)) {
            this.level++; this.score+=500*this.level; this.onScore(this.score);
            this._genLevel(); this._resetBall(); this.state=this.STATE.WAIT;
            this.flashA=0.22; this.flashC='#00FF88'; this.shakeT=8; this._sfx('levelUp');
    ...
    _gameOver() {
        this.state=this.STATE.DEAD; this.deathA=0;
        if(this.score>this.bestScore){this.bestScore=this.score;localStorage.setItem('bvb2_best',this.bestScore);}
    ...
    _burst(x, y, col, n) {
        for(let i=0; i<n && this.parts.length<this.MAX_PARTS; i++) {
            const a=Math.random()*Math.PI*2, sp=Math.random()*4.5+1.5;
            this.parts.push({x,y,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp,r:Math.random()*3.5+1.5,life:1,dec:0.04,col,g:0.1});
        }
    }
    ...
    draw(ts) {
        ...
        if (!this.isMobile) {
            ...
            vg.addColorStop(0,'transparent'); vg.addColorStop(1,'rgba(0,0,5,0.45)');
    ...
    _drawBG() {
        ...
        for(let i=0;i<this.stars.length;i+=step){
            ...
            ctx.globalAlpha=0.1+((Math.sin(s.ph)+1)/2)*0.4;
    ...
    _drawBlocks(ts) {
        ...
        this.blocks.forEach(bl => {
            ...
            const cx=bl.x+bl.w/2, cy=bl.y+bl.h/2;
            ctx.save(); ctx.translate(this.X(cx),this.X(cy));
            ...
            bg.addColorStop(0,bl.hitAnim>0.5?'#fff':bc.lt); bg.addColorStop(0.5,bc.fill); bg.addColorStop(1,bc.dk);
            ctx.fillStyle=bg;
            ctx.beginPath();
            const dw=this.S(bl.w),dh=this.S(bl.h),dr=this.S(4);
            ctx.roundRect(this.S(-bl.w/2),this.S(-bl.h/2),dw,dh,dr); ctx.fill();
            ctx.strokeStyle=`rgba(255,255,255,${bl.hitAnim>0?0.5:0.12})`;
            ctx.lineWidth=this.S(0.8); ctx.stroke();
            if(bl.hp>1&&!bl.dead){
    ...
    /* ═══ UPDATED _drawPaddle — no trail on mobile ═══ */
    _drawPaddle(ts) {
        const ctx=this.ctx, p=this.paddle;
        // Trail — skip on mobile
        if (!this.isMobile) {
            p.trail.forEach((t,i) => {
                const ratio=i/p.trail.length;
                ctx.globalAlpha=ratio*0.12; ctx.fillStyle='#00D4FF';
                this._rrect(t.x-p.w*ratio*0.4,p.y-p.h/2,p.w*ratio*0.8,p.h,6); ctx.fill();
            });
        }
        ctx.globalAlpha=1;
        ...
        ctx.fillStyle=pg; this._rrect(p.x-p.w/2,p.y-p.h/2,p.w,p.h,7); ctx.fill();
        ctx.strokeStyle='rgba(255,255,255,0.22)'; ctx.lineWidth=this.S(1);
        this._rrect(p.x-p.w/2,p.y-p.h/2,p.w,p.h,7); ctx.stroke();
        ctx.fillStyle='rgba(255,255,255,0.26)';
        ctx.fillRect(this.X(p.x-p.w/2+4),this.X(p.y-p.h/2+2),this.S(p.w*0.4),this.S(3));
        for(let i=0;i<this.lives;i++){
            ctx.fillStyle=i===0?'#FF006E':'#FF006E88';
            this._circle(p.x-14+i*14,p.y+p.h/2+10,4); ctx.fill();
        }
    }

    /* ═══ UPDATED _drawBalls — no trail on mobile ═══ */
    _drawBalls(ts) {
        const ctx=this.ctx;
        this.balls.forEach(b => {
            // Trail — skip on mobile
            if (!this.isMobile) {
                b.trail.forEach((t,i) => {
                    ...
                    ctx.globalAlpha=ratio*0.25; ctx.fillStyle='#00D4FF';
                    this._circle(t.x,t.y,b.r*ratio*0.6); ctx.fill();
                });
            }
            ctx.globalAlpha=1;
            ...
            ctx.fillStyle=bg; this._circle(b.x,b.y,b.r); ctx.fill();
            ctx.strokeStyle='rgba(255,255,255,0.28)'; ctx.lineWidth=this.S(1);
            this._circle(b.x,b.y,b.r); ctx.stroke();
        });
    }

    /* ═══ UPDATED _drawRings — no per-ring save/restore ═══ */
    _drawRings() {
        const ctx=this.ctx;
        ctx.save();
        this.rings.forEach(r => {
            ctx.globalAlpha=r.a; ctx.strokeStyle=r.col;
            ctx.lineWidth=this.S(2*r.a); this._circle(r.x,r.y,r.r); ctx.stroke();
        });
        ctx.restore();
    }

    /* ═══ UPDATED _drawParts — batched, no shadow ═══ */
    _drawParts() {
        const ctx=this.ctx, len=this.parts.length;
        if(!len) return;
        ctx.save();
        for(let i=0;i<len;i++){
    ...
    _drawPops() {
        this.pops.forEach(p => {
            this._txt(p.text,p.x,p.y,{sz:12,wt:'bold',col:p.col,al:'center',op:p.op,stroke:true,sc:'rgba(0,0,0,0.5)',sw:2.2,ff:this.FT});
        });
    }
    ...
    _drawHUD(ts) {
        ...
        hg.addColorStop(0,'rgba(0,0,0,0.75)'); hg.addColorStop(1,'rgba(0,0,0,0.05)');
        ctx.fillStyle=hg; ctx.fillRect(0,0,this.canvas.width,this.X(48));
        this._txt(this.score.toLocaleString(),W/2,22,{sz:17,wt:'bold',col:'#fff',al:'center',ff:this.FT});
        if(this.bestScore>0) this._txt(`BEST  ${this.bestScore.toLocaleString()}`,W/2,37,{sz:8,col:'rgba(255,215,0,0.42)',al:'center',ff:this.FU});
        ctx.fillStyle='rgba(0,212,255,0.18)'; ctx.strokeStyle='rgba(0,212,255,0.42)'; ctx.lineWidth=this.S(1);
        ...
        if(this.combo>1){
            this._txt(`×${this.combo} COMBO`,W/2,this.H-65,{sz:13,wt:'bold',col:'#FFD700',al:'center',op:Math.min(1,0.5+Math.sin(ts/200)*0.5),stroke:true,sc:'rgba(0,0,0,0.5)',sw:2,ff:this.FT});
        }
    }
    ...
    _drawFSBtn(ts) {
        ...
        ctx.fillStyle='rgba(0,0,10,0.52)'; ctx.strokeStyle='rgba(255,255,255,0.18)'; ctx.lineWidth=this.S(1.2);
    ...
    _drawWait(ts) {
        ...
        this.ctx.strokeStyle='rgba(0,212,255,0.32)'; this.ctx.lineWidth=this.S(1.5);
        ...
        this._txt('Tap to launch ball',cx,cy+ch/2-10+bob,{sz:10,col:`rgba(0,212,255,${0.4+Math.sin(this.time/440)*0.4})`,al:'center',ff:this.FU});
    }
    ...
    _drawDeath(ts) {
        ...
        ctx.fillStyle='rgba(6,2,18,0.97)'; this._rrect(px,py,pw,ph,20); ctx.fill();
        ctx.strokeStyle='rgba(0,180,255,0.42)'; ctx.lineWidth=this.S(1.5); this._rrect(px,py,pw,ph,20); ctx.stroke();
        ctx.fillStyle='rgba(0,180,255,0.08)'; ctx.fillRect(this.X(px),this.X(py),this.X(pw),this.S(3));
        ctx.globalAlpha=1;
    ...
    /* ═══ UPDATED _loop — 60fps cap ═══ */
    _loop(ts) {
        if(this.destroyed) return;
        const dt=Math.min(ts-(this.lastTime||ts),50);
        // 60fps cap
        if(dt<14){ this.animId=requestAnimationFrame(t=>this._loop(t)); return; }
        this.lastTime=ts;
        this.update(ts,dt);
        this.draw(ts);
        this.animId=requestAnimationFrame(t=>this._loop(t));
    ...
    resize() {
        ...
        this.paddle.x=Math.min(this.paddle.x,this.W-this.paddle.w/2-5);
        this.COLS=Math.min(8,Math.floor((this.W-20)/48));
        this.stars=this._mkStars(this.isMobile?15:35);
    }
