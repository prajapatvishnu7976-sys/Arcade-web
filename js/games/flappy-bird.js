...
class FlappyBird {
    constructor(canvas, onScore) {
        ...
        // ✅ FIX: Use real DPR for EVERYTHING — no blur
        // Mobile pe cap at 2, desktop pe cap at 2.5
        ...
        this.floatTexts = [];
        this.rings      = [];
        // ✅ Reduced particles on mobile for perf with real DPR
    ...
    // ✅ All pixel math uses REAL DPR now — no blur
    ...
    drawText(ctx, text, x, y, opts = {}) {
        const {
            ...
            stroke = false, strokeColor = 'rgba(0,0,0,0.88)', strokeWidth = 3,
            opacity = 1,
        } = opts;
        ...
        // ✅ Same DPR for text and game — both HD now
        const fontSize = Math.round(size * this.dpr);
    ...
    prebuildBird() {
        ...
        const bg = bx.createRadialGradient(cx - dr*0.3, cy - dr*0.3, dr*0.05, cx, cy, dr);
        bg.addColorStop(0, '#88F0FF'); bg.addColorStop(0.4, '#00D4FF'); bg.addColorStop(1, '#004488');
        bx.fillStyle = bg;
        bx.beginPath(); bx.arc(cx, cy, dr, 0, Math.PI*2); bx.fill();

        bx.strokeStyle = 'rgba(255,255,255,0.6)'; bx.lineWidth = 1.5 * this.dpr; bx.stroke();

        const ex = cx + dr*0.3, ey = cy - dr*0.2;
        bx.fillStyle = '#fff'; bx.beginPath(); bx.arc(ex, ey, dr*0.27, 0, Math.PI*2); bx.fill();
        bx.fillStyle = '#001830'; bx.beginPath(); bx.arc(ex + 1.5*this.dpr, ey, dr*0.13, 0, Math.PI*2); bx.fill();
        bx.fillStyle = 'rgba(255,255,255,0.85)'; bx.beginPath(); bx.arc(ex + 2*this.dpr, ey - 2*this.dpr, dr*0.065, 0, Math.PI*2); bx.fill();

        bx.fillStyle = '#FFB820'; bx.strokeStyle = '#BB8800'; bx.lineWidth = 0.7 * this.dpr;
        bx.beginPath();
        ...
        bx.beginPath(); bx.arc(cx - dr*0.2, cy - dr*0.23, dr*0.3, 0, Math.PI*2); bx.fill();
    ...
    _buildPipeCanvas(theme, pw) {
        const capH = 18, capExt = 4, totalW = pw + capExt * 2;

        const bodyKey = `body_${theme.body}_${pw}`;
        if (!this.pipeCache[bodyKey]) {
            const bc = document.createElement('canvas');
            bc.width = this.dSr(pw); bc.height = 1;
            const bx = bc.getContext('2d');
        ...
        if (!this.pipeCache[capKey]) {
            ...
            cc.width = this.dSr(totalW); cc.height = this.dSr(capH);
            ...
            cx.fillStyle = cg; cx.fillRect(0, 0, cc.width, cc.height);
    ...
    makeClouds() {
        ...
        return Array.from({ length: count }, (_, i) => ({
            ...
            w: Math.random() * 65 + 45, h: Math.random() * 20 + 12,
    ...
    startGame() {
        ...
        this.score = 0; this.pipes = []; this.pipeCount = 0;
        this.pipeSpeed = 2.0;
        this.pipeGap   = this.isMobile ? 165 : 180;
        this.pipeDist  = this.isMobile ? 230 : 260;
        this.particles = []; this.floatTexts = []; this.rings = [];
        this.pipeCache = {};
        this.bird.y = this.H * 0.42; this.bird.vy = 0;
        this.bird.rotation = 0; this.bird.trail = [];
        this.bird.alive = true; this.bird.flapAnim = 0;
    ...
    update(dt) {
        ...
        // ✅ Mobile pe slow updates skip — every 3rd frame
        const doSlow = !this.isMobile || (this.frame % 3 === 0);

        if (doSlow) {
            for (let i = 0; i < this.stars.length; i++) this.stars[i].phase += this.stars[i].speed;
        }
        ...
        for (let i = this.rings.length-1; i >= 0; i--) {
            const rg = this.rings[i]; rg.r += 3.5; rg.opacity -= 0.06;
        ...
        if (doSlow) {
            ...
            for (let i = 0; i < this.clouds.length; i++) {
                const c = this.clouds[i]; c.x -= c.speed * spd;
                if (c.x + c.w < 0) c.x = this.W + c.w;
            }
            for (let i = 0; i < this.bgLayers.length; i++) {
                const b = this.bgLayers[i]; b.x -= b.speed * spd;
        ...
        for (let i = this.pipes.length - 1; i >= 0; i--) {
            ...
            if (!p.scored && p.x + this.pipeW < this.bird.x - this.bird.r) {
                ...
                if (!this.isMobile) this.spawnParticles(this.bird.x, this.bird.y, '#FFD700', 5);
                ...
                if (this.score % 5 === 0) {
                    ...
                    if (!this.isMobile) { this.screenShake.timer = 4; this.screenShake.force = 2; }
                }
            }
    ...
    die() {
        ...
        this.flashAlpha = 0.4; this.flashColor = '#FF0033';
    ...
    spawnParticles(x, y, color, count, opts = {}) {
        ...
        for (let i = 0; i < c; i++) {
            ...
            this.particles.push({
                x, y, vx: Math.cos(a)*sp, vy: Math.sin(a)*sp*0.5+baseVY,
                color, size: Math.random()*3+1.5, life: Math.floor(Math.random()*14+8)
            });
        }
    }
    ...
    drawBackground(ctx) {
        ...
        if (!this.isMobile || this.frame % 3 === 0) {
            for (let i = 0; i < this.stars.length; i++) {
                ...
                ctx.globalAlpha = 0.15 + ((Math.sin(s.phase)+1)*0.5)*0.45;
    ...
    drawCityBg(ctx) {
        for (let i = 0; i < this.bgLayers.length; i++) {
            const b = this.bgLayers[i]; ctx.fillStyle = b.color;
    ...
    drawPipes(ctx) {
        for (let i = 0; i < this.pipes.length; i++) {
            const p = this.pipes[i], th = p.theme, pw = this.pipeW;
    ...
    _drawPipeFast(ctx, px, py, pw, ph, theme, isTop) {
        ...
        if (!this.isMobile) {
            ...
            ctx.shadowBlur = this.dS(6); ctx.shadowColor = theme.glow;
    ...
    drawGround(ctx) {
        ...
        ctx.strokeStyle = 'rgba(185,71,217,0.82)'; ctx.lineWidth = this.dS(2);
        // ✅ No shadow on mobile
        if (!this.isMobile) { ctx.shadowBlur = this.dS(8); ctx.shadowColor = '#B947D9'; }
        ctx.beginPath(); ctx.moveTo(0, this.dY(gy)); ctx.lineTo(this.canvas.width, this.dY(gy)); ctx.stroke();
        ctx.shadowBlur = 0;

        if (!this.isMobile || this.frame % 3 === 0) {
            ctx.globalAlpha = 0.12; ctx.strokeStyle = '#B947D9'; ctx.lineWidth = this.dS(0.6);
    ...
    drawRingsFX(ctx) {
        ...
        for (let i = 0; i < this.rings.length; i++) {
            ...
            ctx.globalAlpha = rg.opacity; ctx.strokeStyle = rg.color;
            ctx.lineWidth = this.dS(1.5 * rg.opacity + 0.5);
            ctx.beginPath(); ctx.arc(this.dX(rg.x), this.dY(rg.y), this.dS(rg.r), 0, Math.PI*2); ctx.stroke();
        }
        ctx.globalAlpha = 1;
    }
    ...
    drawBirdTrail(ctx) {
        ...
        for (let i = 0; i < len; i++) {
            const pt = trail[i], alpha = (1 - i/len) * 0.2, r = this.bird.r * (1 - i/len) * 0.5;
    ...
    _drawBirdAt(ctx, cx, cy, rot, flapAnim, pulseT) {
        const r = this.bird.r, size = this.birdSize;
        ctx.save(); ctx.translate(this.dX(cx), this.dY(cy)); ctx.rotate(rot);

        // ✅ Ring only desktop
        if (!this.isMobile) {
            ctx.strokeStyle = 'rgba(0,212,255,0.28)'; ctx.lineWidth = this.dS(1.5);
            ctx.beginPath(); ctx.arc(0, 0, this.dS(r + 5), 0, Math.PI*2); ctx.stroke();
        }

        if (this.birdCanvas) ctx.drawImage(this.birdCanvas, -size/2, -size/2, size, size);

        const dr = r * this.dpr, wingBaseY = dr * 0.22;
        const wingAng = flapAnim > 0 ? -Math.PI*0.48*flapAnim : Math.PI*0.1*Math.sin(pulseT*2.8);
        ...
        ctx.strokeStyle = 'rgba(0,195,235,0.85)'; ctx.lineWidth = dr * 0.22; ctx.lineCap = 'round';
        ctx.beginPath(); ctx.moveTo(-dr*0.12, wingBaseY);
        ctx.quadraticCurveTo(-dr*0.5, wingBaseY + dr*0.24, wingEX, wingEY); ctx.stroke();
        ctx.lineCap = 'butt';
        ctx.restore();
    }
    ...
    drawMenu(ctx) {
        ...
        this.drawRoundRect(ctx, W/2-155, H*0.12, 310, 105, 16); ctx.fill();
        ctx.strokeStyle = 'rgba(0,212,255,0.2)'; ctx.lineWidth = this.dS(1);
        this.drawRoundRect(ctx, W/2-155, H*0.12, 310, 105, 16); ctx.stroke();

        this.drawText(ctx, 'FLAPPY', W/2, H*0.14+26, {
            size:28, weight:'900', color:'#00D4FF', align:'center', baseline:'middle',
            glow:!this.isMobile, glowColor:'#00D4FF', glowBlur:10,
            stroke:true, strokeColor:'rgba(0,0,0,0.88)', strokeWidth:4, family:this.FONT_TITLE
        });
        this.drawText(ctx, 'NEON BIRD', W/2, H*0.14+60, {
            size:17, weight:'900', color:'#B947D9', align:'center', baseline:'middle',
            glow:!this.isMobile, glowColor:'#B947D9', glowBlur:7,
            stroke:true, strokeColor:'rgba(0,0,0,0.88)', strokeWidth:3, family:this.FONT_TITLE
        });
        this.drawText(ctx, 'CINEMATIC EDITION', W/2, H*0.14+84, {
            size:8, weight:'600', color:'rgba(180,180,220,0.4)', align:'center', baseline:'middle', family:this.FONT_UI
        });

        if (this.bestScore > 0)
            this.drawText(ctx, `BEST: ${this.bestScore}`, W/2, H*0.60, {
                size:13, weight:'800', color:'#FFD700', align:'center', baseline:'middle',
        ...
        this.drawText(ctx, '▶  TAP  TO  FLY', W/2, H*0.70, {
            size:13, weight:'800', color:'#00FF88', align:'center', baseline:'middle',
            glow:!this.isMobile, glowColor:'#00FF88', glowBlur:6, family:this.FONT_TITLE, opacity:pulse
        });
        this.drawText(ctx, 'SPACE / CLICK / TAP', W/2, H*0.76, {
            size:9, color:'rgba(180,180,220,0.3)', align:'center', baseline:'middle', family:this.FONT_UI
        });

        const btnX = W-38, btnY = 14;
        ctx.fillStyle = this.soundEnabled ? 'rgba(0,212,255,0.18)' : 'rgba(255,0,80,0.18)';
        this.drawRoundRect(ctx, btnX, btnY, 28, 22, 6); ctx.fill();
        ctx.strokeStyle = this.soundEnabled ? 'rgba(0,212,255,0.5)' : 'rgba(255,0,80,0.5)';
        ctx.lineWidth = this.dS(1);
        this.drawRoundRect(ctx, btnX, btnY, 28, 22, 6); ctx.stroke();
        this.drawText(ctx, this.soundEnabled ? '🔊' : '🔇', btnX+14, btnY+11, { size:10, align:'center', baseline:'middle' });

        const tips = ['Tap to flap wings', 'Fly through the gaps', "Don't touch the pipes!"];
        for (let i = 0; i < tips.length; i++)
            this.drawText(ctx, tips[i], W/2, H*0.84+i*17, {
                size:9, color:'rgba(155,155,200,0.35)', align:'center', baseline:'middle', family:this.FONT_UI
            });
    }
    ...
    drawParticles(ctx) {
        ...
        for (let i = 0; i < this.particles.length; i++) {
            const p = this.particles[i];
            ctx.globalAlpha = Math.min(1, p.life / 10); ctx.fillStyle = p.color;
            ctx.beginPath(); ctx.arc(this.dX(p.x), this.dY(p.y), Math.max(0.5, this.dS(p.size)), 0, Math.PI*2); ctx.fill();
        }
        ctx.globalAlpha = 1;
    }
    ...
    drawFloatTexts(ctx) {
        for (let i = 0; i < this.floatTexts.length; i++) {
            const t = this.floatTexts[i], sc = Math.min(1, t.scale);
            this.drawText(ctx, t.text, t.x, t.y, {
                size:12*sc, weight:'900', color:t.color, align:'center', baseline:'middle', opacity:t.opacity,
                stroke:true, strokeColor:'rgba(0,0,0,0.65)', strokeWidth:2,
                glow:!this.isMobile, glowColor:t.color, glowBlur:6, family:this.FONT_TITLE
            });
        }
    }
    ...
    drawHUD(ctx) {
        ...
        this.drawText(ctx, String(this.score), W/2, 50, {
            size:32, weight:'900', color:'#fff', align:'center', baseline:'middle',
            glow:!this.isMobile, glowColor:'#00D4FF', glowBlur:8,
            stroke:true, strokeColor:'rgba(0,0,0,0.65)', strokeWidth:4, family:this.FONT_TITLE
        });
        this.drawText(ctx, `BEST:${this.bestScore}`, W-8, 18, {
            size:10, weight:'700', color:'rgba(255,215,0,0.55)', align:'right', baseline:'middle', family:this.FONT_UI
        });
        ...
        this.drawText(ctx, `${this.pipeSpeed.toFixed(1)}x`, 8, 18, {
            size:10, weight:'700', color:speedCol, baseline:'middle', family:this.FONT_UI
        });
    ...
    drawDeathScreen(ctx) {
        ...
        this.drawRoundRect(ctx, cx-155, cy-112, 310, 224, 20); ctx.fill();
        ctx.strokeStyle = 'rgba(255,0,60,0.28)'; ctx.lineWidth = this.dS(1.5);
        this.drawRoundRect(ctx, cx-155, cy-112, 310, 224, 20); ctx.stroke();
        ctx.globalAlpha = 1;

        this.drawText(ctx, 'GAME OVER', cx, cy-74, {
            size:23, weight:'900', color:'#FF006E', align:'center', baseline:'middle',
            glow:!this.isMobile, glowColor:'#FF006E', glowBlur:9, family:this.FONT_TITLE, opacity:cA
        });
        this.drawText(ctx, 'SCORE', cx, cy-33, {
            size:9, weight:'700', color:'rgba(180,180,220,0.5)', align:'center', baseline:'middle', family:this.FONT_UI, opacity:cA
        });
        this.drawText(ctx, String(this.score), cx, cy+5, {
            size:38, weight:'900', color:'#fff', align:'center', baseline:'middle',
            glow:!this.isMobile, glowColor:'#00D4FF', glowBlur:8,
            stroke:true, strokeColor:'rgba(0,0,0,0.6)', strokeWidth:5, family:this.FONT_TITLE, opacity:cA
        });
        this.drawText(ctx, `BEST:  ${this.bestScore}`, cx, cy+45, {
            size:13, weight:'800', color:'#FFD700', align:'center', baseline:'middle',
            glow:!this.isMobile, glowColor:'#FFD700', glowBlur:5, family:this.FONT_TITLE, opacity:cA
        });

        if (this.score > 0 && this.score >= this.bestScore)
            this.drawText(ctx, '★  NEW BEST!  ★', cx, cy+72, {
                size:11, weight:'900', color:'#00FF88', align:'center', baseline:'middle',
                glow:!this.isMobile, glowColor:'#00FF88', glowBlur:6, family:this.FONT_TITLE, opacity:cA
            });

        if (elapsed > 700) {
            if (Math.sin(elapsed / 280) > 0)
                this.drawText(ctx, '▶  TAP TO RESTART', cx, cy+100, {
                    size:12, weight:'800', color:'#00FF88', align:'center', baseline:'middle',
                    glow:!this.isMobile, glowColor:'#00FF88', glowBlur:6, family:this.FONT_TITLE, opacity:cA*0.9
                });
        } else {
            this.drawText(ctx, 'Wait...', cx, cy+100, {
                size:9, color:'rgba(160,160,200,0.22)', align:'center', baseline:'middle', family:this.FONT_UI, opacity:cA
            });
        }
    }

    // ══════════════════════════════════════════
    // GAME LOOP — ✅ Smart adaptive for real DPR
    // ══════════════════════════════════════════
    loop(timestamp) {
        ...
        if (this.isMobile) {
            ...
            if (this.fpsHistory.length === 30) {
                ...
                // ✅ If below 30fps, skip draw every other frame
    ...
    destroy() {
        ...
        this.pipeCache = {}; this.colorCache = {}; this.birdCanvas = null;
    }
}